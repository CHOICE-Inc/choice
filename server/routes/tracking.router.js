...
router.get('/latestSubmission', function(req, res) {
  console.log('in server finding the latest submission');
  console.log('goal id is ', req.params.id);
  ...
  pool.connect(function(err, client, done, next) {
    ...
    //join goal, client, staff, job, job_site to find all goal date
    client.query("select am_or_pm, date_tracked from goal_tracking where goal_id = 1 order by id desc limit 1;",
    [goal_id],
        function (err, result) {
          done();
          if(err) {
